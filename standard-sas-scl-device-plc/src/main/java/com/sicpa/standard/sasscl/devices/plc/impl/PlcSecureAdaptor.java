...
public class PlcSecureAdaptor extends PlcAdaptor {
	...
	// TODO do the injection from spring
	protected PlcVariable<Integer> userIdAuthVar;
	protected PlcVariable<Integer> userJavaAccessVar;
	protected PlcVariable<Integer> userPrinterAccessVar;
	protected String userAuthVarName;
	...
	public enum SecurePlcRequest {
		...
		OPEN_ELECTRICAL_CABINET, OPEN_PRINTER_CABINET, OPEN_DEPORTED_PRINTER_CABINET, REBOOT_SECURE_SYSTEM, LSM_TAKE_PICTURE, SECURITY_ALARM, RELOAD_USER_DATA_FILE, SET_USER_ID, AUTHENTICATE_USER;
	}
	...
	protected void onPlcConnected() {
		...
		addPlcListener(new IPlcListener() {
			...
			public List<String> getListeningVariables() {
				...
				result.addAll(PlcVariableMap.getLinesVariableName(userAuthVarName));
				return result;
			}
		});
	...
	protected void handleEvent(final PlcEvent event) {
		...
		if (event.getVarName().equals(userAuthVarName)) {
	...
	/**
	 * Sends the PLC requests due to the login attempt event from the GUI Checks if the password is correct before (...
	 * shouldn't be done here, it's due to the way the SecurityService is implemented)
	 * 
	 * @param event
	 */
	@Subscribe
	 ...
	 * otherwise retrieves the UserId associated Returns the UserId object associated
	 * 
	 * @return
	...
	protected UserId checkPlcUser() throws PlcAdaptorException {
		...
		try {
			intuserId = read(userIdAuthVar);
			OperatorLogger.log("Plc User id received: {}", intuserId);

			javaAccess = read(userJavaAccessVar);
			printerAccess = read(userPrinterAccessVar);
	...
	private User getAppUser(UserId userId) throws PlcAdaptorException {
		...
		for (User user : securityModel.getUsers()) {
			...
			try {
				...
				throw new PlcAdaptorException("Invalid application user profile id (should be a digit) : "
						+ user.getLogin());
			}
	...
	public void setUserIdAuthVar(PlcVariable<Integer> userIdAuthVar) {
		this.userIdAuthVar = userIdAuthVar;
	}

	public void setUserJavaAccessVar(PlcVariable<Integer> userJavaAccessVar) {
		this.userJavaAccessVar = userJavaAccessVar;
	}

	public void setUserPrinterAccessVar(PlcVariable<Integer> userPrinterAccessVar) {
		this.userPrinterAccessVar = userPrinterAccessVar;
	}

	public void setUserAuthVarName(String userAuthVarName) {
		this.userAuthVarName = userAuthVarName;
	}
}
