...
public class CoverFlow extends JPanel {
	...
	private JScrollBar getScrollBar() {
		if (this.scrollBar == null) {
			...
			this.scrollBar.putClientProperty(SicpaScrollBarUI.UNIT_INCREMENT_AUTO, Boolean.FALSE);
		...
		public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
			ThreadUtils.invokeLater(new Runnable() {
				...
				public void run() {
					...
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex).setAnimProgress(
							timelinePosition * angleLeft + angleRight);
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex - 1).setAnimProgress(
