 ...
 * @(#)MeasuredShape.java
 *
 * $Date: 2010-03-19 18:53:26 -0500 (Fri, 19 Mar 2010) $
 *
...
public class MeasuredShape {
	...
	public static MeasuredShape[] getSubpaths(PathIterator i,float spacing) {
		...
		return (MeasuredShape[])v.toArray(new MeasuredShape[v.size()]);
	}
	...
	public MeasuredShape(PathIterator i,float spacing) {
		...
		while(i.isDone()==false) {
			...
			if(k==PathIterator.SEG_CLOSE) {
				...
				v.add(s);
				totalDistance += s.realDistance;
			}
			i.next();
		}
		float t = totalDistance;
		if(v.size()>0) {
			Segment last = (Segment)v.get(v.size()-1);
		...
		segments = (Segment[])v.toArray(new Segment[v.size()]);
		//normalize everything:
		for(int a = 0; a<segments.length; a++) {
			segments[a].normalizedDistance = segments[a].realDistance/totalDistance;
		}
	}
	...
	/** The distance this path covers
	 * 
	 * @return the distance this path covers
	 */
	 ...
	 * <P>The difference between this value and the value returned by
	 * <code>getDistance()</code> is this value <i>may</i> represent
	 * an unclosed shape.  The value returned by <code>getDistance()</code>
	 * represents the total circumference of this shape.
	 * If the original shape this object was constructed from was not closed,
	 * then this value will represent the length of that unclosed shape.
	 * 
	...
	public Point2D getPoint(float distance,Point2D dest) {
		...
		if(distance>totalDistance) throw new IllegalArgumentException("distance ("+distance+") must not be greater than the total distance of this shape ("+totalDistance+")");
	...
	public GeneralPath getShape(float position,float length) {
		GeneralPath dest = new GeneralPath(GeneralPath.WIND_NON_ZERO);
