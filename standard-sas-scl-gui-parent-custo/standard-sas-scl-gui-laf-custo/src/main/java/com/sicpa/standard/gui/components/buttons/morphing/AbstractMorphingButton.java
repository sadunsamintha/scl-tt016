package com.sicpa.standard.gui.components.buttons.morphing;
...
public abstract class AbstractMorphingButton extends JButton {
	...
	private final static SubstanceFillPainter painter = new StandardFillPainter();// SubduedGradientPainter
	...
	protected void paintComponent(final Graphics g) {
		...
		if (isEnabled()) {
			...
			g2.setComposite(AlphaComposite.SrcOver.derive(this.morphing));

			painter.paintContourBackground(g2, this, getWidth(), getHeight(), this.morph, false, SubstanceLookAndFeel
					.getCurrentSkin().getColorScheme(this, ComponentState.ARMED), true);

		} else {
			painter.paintContourBackground(g2, this, getWidth(), getHeight(), this.sourceShape, false,
					SubstanceLookAndFeel.getCurrentSkin().getColorScheme(this, ComponentState.DISABLED_DEFAULT), true);
		}
	}
