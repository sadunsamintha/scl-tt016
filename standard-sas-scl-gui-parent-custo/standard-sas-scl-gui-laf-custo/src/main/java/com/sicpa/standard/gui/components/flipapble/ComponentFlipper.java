...
public class ComponentFlipper {
	...
	private int extraSpace = 50;// modify the perspective
	...
	public void flip(final JComponent frontComponent, final JComponent backComponent, final JLayeredPane layeredPane) {
		...
		this.renderComp.interImage = GraphicsUtilities.createCompatibleTranslucentImage(this.frontImage.getWidth(),
				this.frontImage.getHeight() + (this.extraSpace * 2));
	...
	private void doSizing() {
		...
		int maxHeight = Math.max(this.frontComponent.getHeight(), this.backComponent.getHeight()) + this.extraSpace;
	...
	private class RenderComponent extends JComponent {
		...
		public void paintComponent(final Graphics g) {
			...
			int y0 = ComponentFlipper.this.extraSpace - this.y;

			Graphics2D g2 = (Graphics2D) g.create();
			g2.setClip(imageX + x0, imageY + y0, this.interImage.getWidth() - (2 * x0) - 1,
					(this.interImage.getHeight() - (2 * y0)));
			if (this.rads < Math.PI / 2) {
				g2.drawImage(this.interImage, imageX + x0, imageY + y0, imageX + x0 + this.interImage.getWidth(),
						imageY + y0 + this.interImage.getHeight(), 0, 0, this.interImage.getWidth(),
						this.interImage.getHeight(), null);
			} else {
		...
		private void updateImage() {
			...
			GradientPaint gp = new GradientPaint(0, 0, new Color(0, 0, 0, 0), image.getWidth(), 0, new Color(0, 0, 0,
					alpha));
			g2.setPaint(gp);
			g2.fillRect(0, ComponentFlipper.this.extraSpace / 2, image.getWidth(), image.getHeight()
					- ComponentFlipper.this.extraSpace);
			g2.dispose();
			...
			this.y = (int) (((ComponentFlipper.this.extraSpace / 2) * Math.sin(this.rads)));
