 ...
 * certain time to be dispatched. <p/> The principle is to record the time at
 * which we start processing an event, and have another thread check frequently
 * to see if we're still processing. If the other thread notices that we've been
 ...
 * finishes. <p/> This is useful in determining what code is causing your Java
...
public final class EventDispatchThreadHangMonitor extends EventQueue {
	...
	private void debug(final String which) {
		if (false) {
			for (int i = this.dispatches.size(); i >= 0; --i) {
				System.out.print(' ');
			}
