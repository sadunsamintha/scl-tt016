...
public class SparkLine extends JComponent {
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				final SparkLine sl = new SparkLine();
					...
					public void run() {
						while (true) {
							...
							SwingUtilities.invokeLater(new Runnable() {
								@Override
								public void run() {
									sl.addDataPoint(r.nextInt(10) > 5 ? 1 : 0, System.currentTimeMillis());
	 ...
	 * @param DATA
	 * @param TIMESTAMP
	 *            (type long like you get it with System.currentTimeMillis())
	 */
	...
	public double getQ3() {
		if (DATA_LIST.size() > 2) {
			...
			return (sortedList.subList(startIndex, sortedList.size() - 1)).get(((sortedList.subList(startIndex,
					sortedList.size() - 1)).size() / 2));
		}
		return 0;
	}
	...
	/**
	 * Returns the timestamp of the first value in the list of datapoints as a long.
	 * 
	...
	/**
	 * Returns the timestamp of the the last value in the list of datapoints as a long.
	 * 
	...
	/**
	 * Returns the custom sparkline color of type java.awt.Paint which will be used if the sparkLineColor is set to
	 * eu.hansolo.steelseries.tools.LcdColor.CUSTOM
	 * 
	...
	/**
	 * Returns true if the hi/lo indicators are visible. They will be disabled automaticaly if smoothing is applied to
	 * the data.
	 * 
	...
	/**
	 * Enables or disables the visibility of the info labels. They won't be drawn if the sparkline is too small.
	 * 
	...
	/**
	 * Defines the color that will be used for the normal area. The given color will be used with an alpha of 0.2f
	 * 
	...
	/**
	 * Calculates the sparkline with all it's parameters. This methods will be called everytime a new value was added to
	 * the DATA_LIST
	 * 
	 * @param WIDTH
	 * @param HEIGHT
	...
	private void calculate(final int WIDTH, final int HEIGHT) {
		...
		if (DATA_LIST.size() > 5 && smoothing) {
			...
			for (int index = 0; index < SIZE; index++) {
				pointList.add(new Point2D.Double((DATA_LIST.get(index).getTimeStamp() - DATA_LIST.getFirst()
						.getTimeStamp()) * pixelResolution,
						((DATA_LIST.get(index).getValue() - lo) * (1 / scaleY) + offset)));
			}
		}
		...
		if (averageVisible || normalAreaVisible) {
			...
			NORMAL_AREA.setRect(INNER_BOUNDS.x, AVERAGE - STANDARD_DEVIATION, INNER_BOUNDS.width,
	...
	/**
	 * Calls the selected smoothing functions and fills the POINT_LIST with the smoothed data
	 */
	private void smoothData() {
		...
		case CONTINUOUS_AVERAGE:
			...
			for (int i = 1; i < SIZE - 1; i++) {
				// System.out.println((a * dataList.get(i - 1) + b * dataList.get(i) + c * (dataList.get( i + 1))) / (a
				// + b + c));
				y = continuousAverage(DATA_LIST.get(i - 1).getValue(), DATA_LIST.get(i).getValue(), DATA_LIST
						.get(i + 1).getValue());
				pointList.add(new Point2D.Double(
						(DATA_LIST.get(i).getTimeStamp() - DATA_LIST.getFirst().getTimeStamp()) * pixelResolution,
						((y - lo) * (1 / scaleY) + offset)));
			}

			// Add last point
			pointList.add(new Point2D.Double((DATA_LIST.getLast().getTimeStamp() - DATA_LIST.getFirst().getTimeStamp())
					* pixelResolution, ((DATA_LIST.getLast().getValue() - lo) * (1 / scaleY) + offset)));
			break;

		case CUBIC_SPLINE:
			for (int i = 2; i < SIZE - 1; i++) {
				y = cubicInterpolate(DATA_LIST.get(i - 2).getValue(), DATA_LIST.get(i - 1).getValue(), DATA_LIST.get(i)
						.getValue(), DATA_LIST.get(i + 1).getValue(), 0.5);
				pointList.add(new Point2D.Double(
						(DATA_LIST.get(i).getTimeStamp() - DATA_LIST.getFirst().getTimeStamp()) * pixelResolution,
						((y - lo) * (1 / scaleY) + offset)));
			}
		...
		case HERMITE:
			for (int i = 2; i < SIZE - 1; i++) {
				y = hermiteInterpolate(DATA_LIST.get(i - 2).getValue(), DATA_LIST.get(i - 1).getValue(),
						DATA_LIST.get(i - 0).getValue(), DATA_LIST.get(i + 1).getValue(), 0.5, 0, 0);
				pointList.add(new Point2D.Double(
						(DATA_LIST.get(i).getTimeStamp() - DATA_LIST.getFirst().getTimeStamp()) * pixelResolution,
						((y - lo) * (1 / scaleY) + offset)));
			}
	...
	private void calcInnerBounds() {
		...
		INNER_BOUNDS.setBounds(INSETS.left, INSETS.top, getWidth() - INSETS.left - INSETS.right, getHeight()
	...
	private BufferedImage create_SPARK_LINE_BACKGROUND_Image(final int WIDTH, final int HEIGHT) {
		...
		final Color[] BACKGROUND_COLORS = { new Color(0.4f, 0.4f, 0.4f, 1.0f), new Color(0.5f, 0.5f, 0.5f, 1.0f),
				new Color(0.5f, 0.5f, 0.5f, 1.0f), new Color(0.9f, 0.9f, 0.9f, 1.0f) };
		...
		final RoundRectangle2D BACKGROUND = new RoundRectangle2D.Double(0, 0, IMAGE_WIDTH, IMAGE_HEIGHT,
		...
		final Color[] FOREGROUND_COLORS = { sparkLineColor.GRADIENT_START_COLOR,
				sparkLineColor.GRADIENT_FRACTION1_COLOR, sparkLineColor.GRADIENT_FRACTION2_COLOR,
				sparkLineColor.GRADIENT_FRACTION3_COLOR, sparkLineColor.GRADIENT_STOP_COLOR };

		if (customSparkLineColor != null && sparkLineColor == LcdColor.CUSTOM) {
			G2.setPaint(customSparkLineColor);
		} else {
			final LinearGradientPaint FOREGROUND_GRADIENT = new LinearGradientPaint(FOREGROUND_START, FOREGROUND_STOP,
		...
		final RoundRectangle2D FOREGROUND = new RoundRectangle2D.Double(1, 1, IMAGE_WIDTH - 2, IMAGE_HEIGHT - 2,
				FOREGROUND_CORNER_RADIUS, FOREGROUND_CORNER_RADIUS);
		G2.fill(FOREGROUND);
	...
	/**
	 * Returns a buffered image that contains the sparkline itself. This image will be calculated everytime a new value
	 * was added to the DATA_LIST
	 * 
	 * @param WIDTH
	 * @param HEIGHT
		...
		if (!pointList.isEmpty()) {
			...
			if (filled) {
				...
				if (START_POINT.distance(END_POINT) != 0) {
					float[] fractions = { 0.0f, baseLineFraction, 1.0f };

					// Set appropriate filling gradient
					final Color[] COLORS;
					if (areaFill == ColorDef.CUSTOM) {
						COLORS = new Color[] { customAreaFillTop, customAreaFillBottom, customAreaFillTop };
					} else {
						COLORS = new Color[] { UTIL.setAlpha(areaFill.LIGHT, 0.75f),
								UTIL.setAlpha(areaFill.DARK, 0.0f), UTIL.setAlpha(areaFill.LIGHT, 0.75f) };
					}
					final LinearGradientPaint SPARK_LINE_GRADIENT = new LinearGradientPaint(START_POINT, END_POINT,
							fractions, COLORS);
			...
			if (startStopIndicatorVisible) {
				G2.drawImage(startIndicatorImage, (int) pointList.get(0).getX() - startIndicatorImage.getWidth() / 2,
						HEIGHT - (int) pointList.get(0).getY() - startIndicatorImage.getHeight() / 2, null);
				G2.drawImage(stopIndicatorImage,
						(int) pointList.get(pointList.size() - 1).getX() - stopIndicatorImage.getWidth() / 2,
						HEIGHT - (int) pointList.get(pointList.size() - 1).getY() - stopIndicatorImage.getHeight() / 2,
						null);
			}
			if (hiLoIndicatorVisible) {
				if (loIndex < pointList.size()) {
					G2.drawImage(loIndicatorImage, (int) pointList.get(loIndex).getX() - loIndicatorImage.getWidth()
							/ 2, HEIGHT - (int) pointList.get(loIndex).getY() - loIndicatorImage.getHeight() / 2, null);
				}
				if (hiIndex < pointList.size()) {
					G2.drawImage(hiIndicatorImage, (int) pointList.get(hiIndex).getX() - hiIndicatorImage.getWidth()
							/ 2, HEIGHT - (int) pointList.get(hiIndex).getY() - hiIndicatorImage.getHeight() / 2, null);
				}
			}
	...
	private BufferedImage create_HI_INDICATOR_Image(final int WIDTH) {
		...
		final float[] THRESHOLD_TRIANGLE_FRACTIONS = { 0.0f, 0.3f, 0.59f, 1.0f };
		final Color[] THRESHOLD_TRIANGLE_COLORS = { new Color(82, 0, 0, 255), new Color(252, 29, 0, 255),
				new Color(252, 29, 0, 255), new Color(82, 0, 0, 255) };
		final LinearGradientPaint THRESHOLD_TRIANGLE_GRADIENT = new LinearGradientPaint(THRESHOLD_TRIANGLE_START,
				THRESHOLD_TRIANGLE_STOP, THRESHOLD_TRIANGLE_FRACTIONS, THRESHOLD_TRIANGLE_COLORS);
	...
	private BufferedImage create_LO_INDICATOR_Image(final int WIDTH) {
		...
		final float[] THRESHOLD_TRIANGLE_FRACTIONS = { 0.0f, 0.3f, 0.59f, 1.0f };
		final Color[] THRESHOLD_TRIANGLE_COLORS = { new Color(0, 0, 72, 255), new Color(0, 29, 255, 255),
				new Color(0, 29, 255, 255), new Color(0, 0, 72, 255) };
		final LinearGradientPaint THRESHOLD_TRIANGLE_GRADIENT = new LinearGradientPaint(THRESHOLD_TRIANGLE_START,
				THRESHOLD_TRIANGLE_STOP, THRESHOLD_TRIANGLE_FRACTIONS, THRESHOLD_TRIANGLE_COLORS);
		G2.setPaint(THRESHOLD_TRIANGLE_GRADIENT);
		G2.fill(THRESHOLD_TRIANGLE);
	...
	private BufferedImage create_START_STOP_INDICATOR_Image(final int WIDTH) {
		...
		final Point2D ELLIPSE_CENTER = new Point2D.Double((0.42857142857142855 * IMAGE_WIDTH),
				(0.2857142857142857 * IMAGE_HEIGHT));
		final float[] ELLIPSE_FRACTIONS = { 0.0f, 0.01f, 0.99f, 1.0f };
		final Color[] ELLIPSE_COLORS = { new Color(204, 204, 204, 255), new Color(204, 204, 204, 255),
				new Color(51, 51, 51, 255), new Color(51, 51, 51, 255) };
		final RadialGradientPaint ELLIPSE_GRADIENT = new RadialGradientPaint(ELLIPSE_CENTER,
				(float) (0.5 * IMAGE_WIDTH), ELLIPSE_FRACTIONS, ELLIPSE_COLORS);
	...
	/**
	 * Calling this method will add all relevant imagetypes to the list of image types that are needed to initialize the
	 * component
	 */
	...
	/**
	 * Calling this method will add all imagetypes to the list of imagetypes so that the next time the init method will
	 * be called all images will be recreated
	 */
