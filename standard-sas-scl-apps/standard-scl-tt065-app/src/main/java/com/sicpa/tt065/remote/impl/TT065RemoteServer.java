package com.sicpa.tt065.remote.impl;
...
/**
 * This is a specialized RemoteServer, created to enable monitoring of SCLs Red Lights. It's base on
 * core's {@link com.sicpa.standard.sasscl.devices.remote.impl.RemoteServer} implementation of an AbstractRemoteServer. It contains
 * similar methods with the inclusion of methods to handle Red Lights.
 * It implements {@link ITT065RemoteServer} to enable monitoring of red light occurrences.
 */
public class TT065RemoteServer extends AbstractRemoteServer implements IConnectable, ITT065RemoteServer {
  private static final Logger LOGGER = LoggerFactory.getLogger(TT065RemoteServer.class);
  ...
  private void handleEncoderInfoResult(SicpadataGeneratorInfoResultDto res) {
    for (SicpadataGeneratorInfoResultDto.InfoResult ir : res.getInfoResult()) {
      if (!ir.isInfoSavedOk()) {
  ...
  /**
   * Send a {@link RedLight} to the Remote Server Global Monitoring Tool
   * @param redLight RedLight to be sent
   */

  @Override
  public void sendRedLightInfoToGlobalMonitoringTool(RedLight redLight) {
    final EventDto eventDto = convertToEvent(redLight);
    try {
      remoteServices.registerEvent(eventDto);
    } catch (MonitoringException e) {
      LOGGER.error("Unable to register a new Event to RemoteServices "+redLight, e);
    }
  }

  private EventDto convertToEvent(final RedLight redLight) {
    final EventDto eventDto = new EventDto();
    final EventTypeDto eventTypeDto = new EventTypeDto();
    eventTypeDto.setBusinessCode(redLight.getStateCode());
    eventDto.setEventType(eventTypeDto);
    eventDto.setEventMessage(RedLight.convertToReadableMessage(redLight));
    eventDto.setEventTime(redLight.getStartDate());
    return eventDto;
  }
